module.exports = async page => {
  ...
  const zonajobs_jobs = await page.evaluate(async () => {
    ...
    await parents.forEach(async parent => {
      ...
      const date = parent.querySelector('span.z-fecha.hidden').textContent;
      if (!date.includes('Ayer')) {
        return;
      }
      let json = {};
      json.site = 'Zonajobs';
      json.title = parent.querySelector('.titulo-aviso').textContent;
      json.publisher = parent.querySelector('.nombre').textContent;
